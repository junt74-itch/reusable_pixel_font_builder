...
- 現在の `dist/` は、ライセンスを確認できた 23 フォントの成果物だけを収録しています。
...
KH-Dot、JF-Dot、美咲の TTF は `_font_asset/` に収録しています。04B（`04B_19_.TTF` / `04b_25_.ttf`）はライセンスを確認できないため、元 TTF と生成物のいずれも収録していません。権利確認済みの別ソースを使う場合は、利用者自身の管理する入力として `_font_asset/` に置いてください。
